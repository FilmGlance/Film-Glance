...
| Apr 30, 2026 | **📋 NEXT STEPS:** **(1)** **Merge the staging → main PR** to ship v5.10.35-36 to production. After merge: update §10 row above to "v5.10.36 in production", tag prior CURRENT STATE rows as SUPERSEDED, and pull `main` locally. **(2)** **Phase 2 — comprehensive mobile sweep** across the result-page sections that don't yet have mobile breakpoints. Audit list (each gets one of three patterns — stack, shrink-and-fit, or horizontal-scroll-with-fade): score panel ✓ done in v5.10.36; cast scroll (96px headshots, currently no breakpoint, may need to shrink to 64px); awards (Won-row gold left bar, no breakpoint); production / theatrical run rows (icon + label + value layout, may collapse on narrow); where to watch (StreamingBadge pills, may need to shrink); you might also like (poster grid 2/3 aspect, breakpoint TBD); video reviews (auto-fit grid, looks OK but verify); favs DYM-style cards in detail (already has v5.10.35 ≤640 rule, may need re-tuning). Get audit approval from user before writing code, ship as v5.10.37 in its own PR. **(3)** **Phase 3** — verify `tickerScroll` + `filmScroll` infinite animations actually animate on mobile (suspected frozen due to viewport mask hiding most items); shorten ticker items or narrow the masks if frozen. **(4)** **Phase 4** — codify a formal responsive contract in `tech-specs.md` §11 (breakpoint table: 360 / 480 / 560 / 700 / 860 / 1380; pattern catalog; verification checklist). **(5)** **Audit any remaining inline `opacity: 0` + animation pairs** — same landmine as the reduced-motion bug fixed in v5.10.36. Check DYM cards, fav cards, score panel, hero items, auth modal, etc. for `opacity: 0` inline that depends on a softFade/fadeIn animation, and ensure each is overridden in the reduced-motion @media block. **(6)** **Last-minute movie page fixes** still queued from real-world v5.10.29 testing — capture details at start of next session. **(7)** **Box Office Totals page** — new top-level page, requirements TBD. **(8)** **Check VPS forum import status** — ETA mid-May. **(9)** **Optional follow-up to v5.10.33 hero static fix** — if title "animating up" persists after merge, root is FOUT (Google Fonts swap reflow); mitigation: `next/font/google` + size-adjust, or change `display=swap` → `display=optional`. — **Also still queued (lower priority):** post-import cleanup (delete `components/ui/floating-particles.tsx`, drop `three` dep, remove `components/preview-landing.jsx`); doubled-log fix on next import stop; KVM 4 → KVM 2 downgrade after import ($15/mo savings); post-import forum queue (GDPR removal, mobile audit, API health, Discuss links, staging cleanup, Capacitor mobile app); 6 Dependabot vulnerabilities (3 high, 3 moderate); rotate Supabase PAT before Apr 17, 2027; delete dead `YOUTUBE_API_KEY` from Vercel; reconstruct missing `003_anonymous_searches.sql` migration; full Stripe teardown (optional); 2026-05-12 13:00 UTC scheduled cleanup agent (`trig_01XgUj4SH6z6d9vSp9Betg8R`) fires; restart Claude Code to activate `huashu-design` skill. | — | — |
