...
- **Navigation links MUST stay reachable on every viewport.** Never write `@media (max-width: ...) { .nav-foo-btn { display: none } }` for a primary nav destination. The canonical mobile collapse is icon-only — hide the LABEL inside the link (`.nav-forum-label { display: none }` at ≤520px is the existing pattern), tighten button padding/gap to fit. Any user must be able to reach Discussion Forum, Box Office, Favourites, and Sign In from a 360px iPhone without typing a URL. **Repeat violation in v5.12.0 (May 2, 2026)** added `.nav-discuss-btn` and `.nav-boxoffice-btn` `display: none` rules at ≤560px — fixed in v5.12.1 by removing those rules and relying on the existing icon-only collapse. Pre-commit check on any nav change: grep for `\.nav-[a-z-]+\s*\{[^}]*display:\s*none` and treat any hit as a regression.
