  ...
  for (const entry of expired) {
    ...
    try {
      ...
      if (tmdb) {
        ...
        if ((tmdb as any).recommendations?.length > 0) {
          mv.recommendations = (tmdb as any).recommendations;
        }
        if ((tmdb as any).video_reviews?.length > 0) {
