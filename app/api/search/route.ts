...
function claudeUserPrompt(title: string, year?: number): string {
  ...
  return `Movie: "${title}"${yearStr}\n\nReturn JSON with: title (official title), year, genre (string like "Action · Comedy"), director, runtime (string like "93 min"), tagline, description, cast (6-8 with name and character), sources (all 9: RT Critics, RT Audience, Metacritic Metascore, Metacritic User, IMDb, Letterboxd, TMDB, Trakt, Simkl — each with name, score as NUMBER, max as NUMBER, type, url), hot_take (object with "good": array of 3 short strings summarizing general positive sentiment about the film, and "bad": array of 3 short strings summarizing general negative sentiment — keep each point to one succinct line, NO SPOILERS, never reveal plot points or endings), awards (IMPORTANT: always populate this array — list ALL major awards including Oscar, Golden Globe, BAFTA, SAG, Cannes, Critics Choice, etc. Each entry: award as string like "Academy Awards", result as "Won" or "Nominated", detail as string like "Best Picture", year as number like 2009. Include both wins AND nominations. Do NOT return an empty array for any movie that has received nominations or wins), boxOffice (budget as "$200,000,000", budgetRank as "#X all-time" or null, openingWeekend as "$128,122,480", openingRank as "#X all-time" or null, pta as "$XX,XXX" per-theater average, ptaRank as "#X all-time" or null, domestic as dollar string, domesticRank as "#X all-time" or null, international as dollar string, internationalRank as "#X all-time" or null, worldwide as dollar string, worldwideRank as "#X all-time" or null, roi as "XXX%" estimated return on investment, roiRank as "#X all-time" or null, theaterCount as number string like "4,662", theaterCountRank as "#X widest release" or null, daysInTheater as "XX days", daysInTheaterRank as "#X longest run" or null. ALWAYS provide ranks for ALL gross fields — opening, domestic, international, worldwide — and budget. If exact rank is unknown, give a reasonable bracket like "Top 5%" or "#1,200+". Only return null if the film genuinely had no theatrical release.). ${year ? `The film is from ${year} — if you don't recognize it, return {"error": "not_a_movie"} so we can fall back to verified data; do NOT substitute a same-titled film from another year. ` : ""}ONLY JSON.`;
}
